...
library(stringr)
library(sf)
##\\central\OAS\SCISRV\rs454\bear\registration   bear harvest filepath?
...
joint <- nimble::nimbleCode( {
  ...
  for(i in 1:ncell){
    for(yr in 1:nyears[i]){
    log(lambda[i,yr]) <- s[i] + b_forest*forest[i] + b_imperv*imperv[i]
    n[i,yr] ~ dpois(lambda[i,yr])
    }
  }
  
  # Camera submodel
  for(j in 1:ncams){
    for(yr in 1:nyears[j]){
      for(k in 1:nsurveys[j,yr]){
      muy[j, yr, k] <- 1 - pow(rho[j, yr, k], n[cell[j]])
      logit(rho[j, yr, k]) <- a_forest*cam_can[j] + a_yday*yday[j, k] + a_yday2*yday2[j, k] + a_height*cam_height[j] + a_dist*cam_dist[j] + eps_p[j, k]
      y[j, yr, k] ~ dbern(muy[j, yr, k])
    }}}
  
  # Harvest submodel
  for(c in 1:ncounty){
    for(yr in 1:nyears[c]){
    log(lambda_county[c, yr]) <- gamma0 + gamma1*log(sum(lambda[low[c,yr]:high[c,yr]])) 
    harvest[c, yr] ~ dpois(effort[c, yr]*lambda_county[c, yr])
  }}
